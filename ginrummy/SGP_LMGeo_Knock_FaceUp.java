 ...
 */
public class SGP_LMGeo_Knock_FaceUp implements GinRummyPlayer {
...
//	}
	...
	public boolean willDrawFaceUpCard(Card card) {
		...
		double maxDeadwood = Double.MIN_VALUE;
		
		if (gamestateIndex <= 4)
		{
			// 1. Return true if card would be a part of a meld, false otherwise.
			this.faceUpCard = card;
			ArrayList<Card> newCards = (ArrayList<Card>) cards.clone();
			newCards.add(card);
			for (ArrayList<Card> meld : GinRummyUtil.cardsToAllMelds(newCards))
		...
		// 2. Compute the intial prediction
//		System.out.println(">>>>>>>>>>> Initial hand0: " + cards);
		ArrayList<Integer> initial_features = new ArrayList<Integer>();
		ArrayList<ArrayList<ArrayList<Card>>> initial_bestMeldSet = GinRummyUtil.cardsToBestMeldSets(cards);
		ArrayList<ArrayList<Card>> initial_bestMelds = new ArrayList<ArrayList<Card>>();
		if (!initial_bestMeldSet.isEmpty()) initial_bestMelds = initial_bestMeldSet.get(0);
		
		// 2.1. Dummy Hand0
		int[] initial_geoRelation = FeatureEngineer0.geoRelation(cards);
		for (int i: initial_geoRelation) initial_features.add(i);
					
		// 2.2. Deadwood
		int initial_deadwood = GinRummyUtil.getDeadwoodPoints(initial_bestMelds, cards);
		initial_features.add(initial_deadwood);
		
		double dwTolerate;
		if (gamestateIndex <= 4) dwTolerate = 0;
		else dwTolerate = -15;
		
		double initial_predScore = linearRegression(initial_features, dwTolerate);
//		System.out.println(">>>>>>>>>>> initial_predScore: " + initial_predScore);
		
		// 3. Compute the best prediction of draw face-up		
		
		// 3.2. Prevent breaking melds
		ArrayList<ArrayList<ArrayList<Card>>> bestMeldSets = GinRummyUtil.cardsToBestMeldSets(newCards);
		...
		else 
		{
			for (Card _card: cards) 
			{
				...
				for (ArrayList<Card> meld: bestMelds)
					if(meld.contains(_card))
					{
						check = true;
						break;
					}
				if (!check) unmeldedCards.add(_card);
			}
		}
		
		// 3.3. If all 10 cards are in melds, but the face-up card couldn't form a melds with current hand, return false
		if (unmeldedCards.size() == 0) return false;
		
//		System.out.println(">>>>>>>>>>> unmeldedCards: " + unmeldedCards);
		
		// 3.4 Compute the best prediction
		for (Card _card: unmeldedCards)
		{
			ArrayList<Card> remainingCards = (ArrayList<Card>) newCards.clone();
			remainingCards.remove(_card);
			...
			{
				if (predScore > maxPredScore) maxPredScore = predScore;
			}
			
			if (deadwood >= maxDeadwood)
			{
				if (deadwood > maxDeadwood) maxDeadwood = deadwood;
			}
			
//			System.out.println(">>>>>>>>>>> predScore: " + predScore + " with " + _card);
//			System.out.println(">>>>>>>>>>> deadwood: " + deadwood + " with " + _card);
		}

		// 4. If the best prediction is draw face-up is better than the best prediction, return true, else return false
		if (maxPredScore > initial_predScore) return true;
		else return false;
		
//		if (maxDeadwood < initial_deadwood) return true;
//		else return false;
	}	
...
//			System.out.println(">>>>>>>>>>>>>> Score: " + predScore);
	...
	{	
		...
		double _8A = -4.9775597361934;
		double _9A = -4.94295589871651;
		double _10A = -1.50670138982543;
		double _11A = -1.8985931419337;
		double _12A = -3.91749768114938;
		double _13A = -4.03271081225038;
		...
		double _8B = -4.33223466771312;
		double _9B = -4.33942014437196;
		double _10B = 0.412352154867484;
		double _11B = -0.802018435873124;
		double _12B = -4.80556177953301;
		double _13B = -4.53533948616175;
		...
		double _8C = -5.50473355977434;
		double _9C = -3.82158089356126;
		double _10C = -0.528528727215937;
		double _11C = -1.43165936761822;
		double _12C = -4.1501793365922;
		double _13C = -3.028721296414;
		...
		double _8D = -5.10127282375423;
		double _9D = -5.21365414710694;
		double _10D = -1.1023454713234;
		double _11D = -1.35248380057135;
		double _12D = -4.2367618568139;
		double _13D = -3.35923048829472;
