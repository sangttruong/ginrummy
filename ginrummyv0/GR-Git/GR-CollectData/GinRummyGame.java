...
public class GinRummyGame {
	private static FileWriter gamestates;
	private static FileWriter features;
	private static int games = 0;
//	private static FileWriter file_buckets;
	...
	public int play() {
		...
		while (scores[0] < GinRummyUtil.GOAL_SCORE && scores[1] < GinRummyUtil.GOAL_SCORE) { // while game not over
			...
			if (DataCollector.player == currentPlayer)
				DataCollector.addGameState(hands.get(DataCollector.player), discards.peek());
			/** added */
			while (deck.size() > 2) { // while the deck has more than two cards remaining, play round
				/** added */
				if (DataCollector.player != currentPlayer)
				{
					DataCollector.addGameState(hands.get(opponent), discards.peek());
					DataCollector.addYouFaceUp(discards.peek());
				}
				else
					DataCollector.addTop(discards.peek());
				/** added */
				...
				if (!(!drawFaceUp && turnsTaken < 2 && faceUpCard == firstFaceUpCard)) { // continue with turn if not initial declined option
					...
					/** added */
					if (DataCollector.player != currentPlayer)
						DataCollector.addYouDrawnFaceUp(drawFaceUp);
					if (DataCollector.player == currentPlayer)
						DataCollector.addDraw(drawCard);
					else
						DataCollector.addYouDraw(drawCard);
					/** added */
					for (int i = 0; i < 2; i++) 
					...
					/** added */
					if (DataCollector.player == currentPlayer)
						DataCollector.addDiscard(discardCard);
					else
						DataCollector.addYouDiscarded(discardCard);
					/** added */
					for (int i = 0; i < 2; i++) 
			...
			if (knockMelds != null) { // round didn't end due to non-knocking and 2 cards remaining in draw pile
				...
				/** added */
				DataCollector.addScore(Math.abs(knockingDeadwood - opponentDeadwood));
				/** added */
				...
				// -1 stands for no Winner/ Knocker/ Scorer
	...
	public static void main(String[] args) throws IOException {
		gamestates = new FileWriter ("gamestates.txt");
		features = new FileWriter("features.txt");
		// file_buckets = new FileWriter("output_buckets.txt");
		...
		/** added */
//		DataCollector.printGameStates(file);
//		DataCollector.generateBuckets();
//		DataCollector.printBuckets(file_buckets);
		DataCollector.printAllFeatures(gamestates, features);
		gamestates.close();
		features.close();
		System.out.println("DONE");
//		XGBPlayer.printHistoryScore();
	}
