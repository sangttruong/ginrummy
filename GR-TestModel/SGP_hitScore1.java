 ...
 * @version 1.0
 */
public class SGP_hitScore1 implements GinRummyPlayer {
...
//		System.out.println("Unmelded cards: " + unmeldedCards);
		...
		// Discard a card in unmelded card so that maximizes the number of hitscore of remaining card --------------------------------------------
		ArrayList<Card> unmeldedCards_maxHitScore = new ArrayList<Card>();
		for (Card card: unmeldedCards)
		{
			...
			if (hitScore >= maxHitScore)
			{
				if (hitScore > maxHitScore) {
					...
					unmeldedCards_maxHitScore.clear();
				}
				unmeldedCards_maxHitScore.add(card);
			}
		}
//		System.out.println("unmeldedCards_maxHitScore: " + unmeldedCards_maxHitScore);
			
		// Discard a card in unmelded_maxHitScore card so that minimize the number of deadwood of remaining card  ------------------------------
		ArrayList<Card> unmeldedCards_maxHitScore_minDeadwood = new ArrayList<Card>();
		for (Card card: unmeldedCards_maxHitScore)
		{
			...
			if (deadwood <= minDeadwood) {
				if (deadwood < minDeadwood) {
					...
					unmeldedCards_maxHitScore_minDeadwood.clear();
				}
				unmeldedCards_maxHitScore_minDeadwood.add(card);
			}
		}
//		System.out.println("unmeldedCards_maxHitScore_minDeadwood: " + unmeldedCards_maxHitScore_minDeadwood);
		
		//--------------------------------------------------------------------------------------------------------------------------------------
		
		Card discard = unmeldedCards_maxHitScore_minDeadwood.get(random.nextInt(unmeldedCards_maxHitScore_minDeadwood.size()));
