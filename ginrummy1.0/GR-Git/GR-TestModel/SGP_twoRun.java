 ...
 * @version 2.0
 */
public class SGP_twoRun implements GinRummyPlayer {
...
// Prevent breaking a two-run ---------------------------------------------------------------------------------------------------------------------------------------------
		boolean[][] chosen = new boolean[4][13];
		ArrayList<Object> temp = FeatureEngineer0.twoRun(cards, chosen);
		ArrayList<Card> twoRun = (ArrayList<Card>) temp.get(1);
		// System.out.println("Two-run of player 0: " + twoRun);
		
		ArrayList<Card> candidateCardsTwoRun = new ArrayList<Card>();
		for(Card card: candidateCards) {
			if (!twoRun.contains(card)) candidateCardsTwoRun.add(card);
		}
		
		Card discard;
		if (!candidateCardsTwoRun.isEmpty()) {
			discard = candidateCardsTwoRun.get(random.nextInt(candidateCardsTwoRun.size()));
		}
		else {
