...
public class GinRummyUtil {
	...
	// Debugging method ----------------------------------------------------------
	public static long[] getCardBitStrings() {
		return cardBitstrings;
	}
	// ---------------------------------------------------------------------------
	...
	// Debugging method ----------------------------------------------------------
	public static ArrayList<ArrayList<Long>> getMeldBitstrings() {
		return meldBitstrings;
	}
	// ---------------------------------------------------------------------------
...
//			System.out.println(meldSet);
			...
			for (int meldIndex : meldIndexSet)
				meldSetBitstring |= meldBitstrings.get(meldIndex); // gop tat ca melds
			...
			for (int i = 0; i < meldBitstrings.size(); i++) { 
				...
				if ((meldSetBitstring & meldBitstring) == 0) { // check if current meld DOES EXIST in the set of melds generated by given cards
					isMaximal = false;
			...
			if (isMaximal) { // muon tim mot luong lon nhat to hop cac melds unique voi nhau
